...
<head>
    ...
    <title>@yield('title', 'SIG Irigasi Sidoarjo')</title>
    ...
    <header>
        <div class="header-container">
            ...
            <nav>
                ...
                <a href="{{ route('peta.index') }}" class="{{ request()->routeIs('peta.*') ? 'active' : '' }}">Peta Interaktif</a>
                <a href="{{ route('data-irigasi.index') }}" class="{{ request()->routeIs('data-irigasi.*') ? 'active' : '' }}">Data Irigasi</a>
            ...
            <div class="nav-auth-area">
                @guest('web')
                    <a href="{{ route('login') }}" class="btn-login"><i class="fas fa-sign-in-alt"></i> Masuk</a>
                @else
                    <a href="{{ route('profil.index') }}" class="user-avatar-btn">
                        <div class="user-avatar">{{ substr(Auth::guard('web')->user()->name, 0, 1) }}</div>
                        <span class="user-name-short">{{ explode(' ', Auth::guard('web')->user()->name)[0] }}</span>
                    </a>
                @endguest
            </div>
        </div>
    ...
    <footer>
        <div class="footer-container">
            ...
            <div class="footer-section">
                ...
                <a href="{{ route('peta.index') }}">Peta Interaktif</a>
                <a href="{{ route('data-irigasi.index') }}">Data Irigasi</a>
                <a href="{{ route('laporan.index') }}">Laporan Kerusakan</a>
            </div>
            <div class="footer-section">
                <h4>Kelola</h4>
                <a href="{{ route('laporan.create') }}">Tambah Laporan</a>
                <a href="{{ route('data-irigasi.index') }}">Saluran Aktif</a>
                <a href="{{ route('laporan.index') }}">Laporan Masuk</a>
            </div>
        </div>
        <div class="footer-bottom">
            <div>&copy; {{ date('Y') }} Sistem Informasi Irigasi - Kabupaten Sidoarjo</div>
